...
function getAllData($table, $where = null, $values = null  , $json = true){
    ...
    if ($json == true ) {
        ...
        if ($count > 0) {
            ...
            return json_encode(array("status" => "No Data PHP"));
        }
    }
}
...
function insertData($table, $data, $json = true)
{
    ...
    if ($json == true) {
    if ($count > 0) {
        echo json_encode(array("status" => "success PHP"));
    } else {
        echo json_encode(array("status" => "failure PHP"));
    }
    }
...
{
    echo     json_encode(array("status" => "failure PHP" , "message" => $message));
}

function   printSuccess($message = "none") 
{
    echo     json_encode(array("status" => "success PHP" , "message" => $message));
}
